...
export class PreThesisService {
  ...
  async getAllTopicsByActiveSemester(): Promise<Topic[]> {
    ...
    if (!activeSemester) throw new AppError('No active semester found', 404);
  ...
  async applyToTopic(
    ...
    if (topic.status === 'closed') throw new AppError('Topic is closed for applications', 400);
    ...
    if (existingApplication && existingApplication.status !== 'cancelled') {
      throw new AppError('Student already applied to this topic', 400);
    }
    ...
    if (hasAccepted) {
      throw new AppError('Student already has an accepted topic application', 400);
    }
    ...
    if (topic.maxSlots !== null) {
      ...
      if (acceptedCount >= topic.maxSlots) {
        throw new AppError('Topic has reached maximum number of students', 400);
      }
    }
    ...
    try {
      ...
      if (status === 'accepted') {
        ...
        if (existingPreThesis.length > 0) {
          await transaction.rollback();
          throw new AppError('Student already has a pre-thesis for this semester', 400);
        }
  ...
  async gradePreThesis(id: number, finalScore: number): Promise<PreThesis | null> {
    if (finalScore < 0 || finalScore > 10) {
      throw new AppError('Final score must be between 0 and 10', 400);
    }
    
    const preThesis = await this.preThesisRepository.findById(id);
    if (!preThesis) {
      throw new AppError('Pre-thesis not found', 404);
    }
