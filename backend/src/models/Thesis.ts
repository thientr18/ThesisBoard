...
export class Thesis extends Model<InferAttributes<Thesis>, InferCreationAttributes<Thesis>> {
  ...
  declare status: 'in_progress' | 'defense_scheduled' | 'defense_completed' | 'completed' | 'cancelled';
}
...
Thesis.init(
  {
    ...
    status: { type: DataTypes.ENUM('in_progress', 'defense_scheduled', 'defense_completed', 'completed', 'cancelled'), defaultValue: 'in_progress' },
  },
  { 
    sequelize,
    tableName: 'theses',
