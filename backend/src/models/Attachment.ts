...
export class Attachment extends Model<InferAttributes<Attachment>, InferCreationAttributes<Attachment>> {
  ...
  declare entityType: 'topic' | 'pre_thesis_project' | 'thesis' | 'submission' | 'announcement';
...
Attachment.init(
  {
    ...
    entityType: {
      type: DataTypes.ENUM('topic', 'pre_thesis_project', 'thesis', 'submission', 'announcement'),
      allowNull: false,
      field: 'entity_type',
    },
  ...
  { sequelize, tableName: 'attachments', modelName: 'Attachment', underscored: true, timestamps: true }
);