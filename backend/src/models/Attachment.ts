...
export class Attachment extends Model<InferAttributes<Attachment>, InferCreationAttributes<Attachment>> {
  ...
  declare entityType: 'topic' | 'submission' | 'announcement' | 'topic_application' | 'thesis_proposal' | 'system' | 'other';
...
Attachment.init(
  {
    ...
    entityType: {
      type: DataTypes.ENUM('topic', 'submission', 'announcement', 'topic_application', 'thesis_proposal', 'system', 'other'),
      allowNull: false,
      field: 'entity_type',
    },
