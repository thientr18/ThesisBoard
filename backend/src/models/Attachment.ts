...
export class Attachment extends Model<InferAttributes<Attachment>, InferCreationAttributes<Attachment>> {
  ...
  declare entityType: 'topic' | 'prethesis_submission' | 'thesis_submission' | 'announcement' | 'topic_application' | 'thesis_proposal' |  'thesis_registration' | 'system' | 'other';
...
Attachment.init(
  {
    ...
    entityType: {
      type: DataTypes.ENUM('topic', 'prethesis_submission', 'thesis_submission', 'announcement', 'topic_application', 'thesis_proposal', 'thesis_registration', 'system', 'other'),
      allowNull: false,
      field: 'entity_type',
    },
